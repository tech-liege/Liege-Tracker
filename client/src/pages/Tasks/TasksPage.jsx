...
export default function TasksPage() {
  ...
  const filteredTodos = useMemo(() => {
    ...
    return todos
      .filter((todo) => {
        ...
        const haystack = [todo.text, ...(todo.tags || [])]
          .join(" ")
          .toLowerCase();
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "priority") {
          ...
          if (aPriority !== bPriority) return (aPriority - bPriority) * direction;
          return compareText(a?.text, b?.text);
        }
  ...
  }, [priorityFilter, search, statusFilter, sortBy, sortOrder, todos]);
    ...
    <div className="flex flex-col gap-8">
      <header className="ui-enter rounded-3xl border border-border bg-white/90 p-6 shadow-soft backdrop-blur sm:p-8">
        ...
        <p className="mt-2 text-sm text-bark">
          Move work through clear states. Keep deadlines, tags, and priorities
          visible.
        </p>
      ...
      <section className="ui-enter-delayed rounded-3xl border border-border bg-white/90 p-6 shadow-soft backdrop-blur sm:p-8">
        ...
        <form onSubmit={handleAdd} className="">
          <fieldset className="grid gap-3 lg:grid-cols-[2fr_180px_170px_1fr_auto] border border-ember rounded-xl p-3 lg:p-5">
        ...
        <fieldset className="mt-6 grid gap-3 lg:grid-cols-[auto_auto_1fr] bg-gray-100 p-3 pt-5 lg:p-5 lg:pt-7 rounded-xl">
          <legend className="relative top-4 font-semibold text-lg">
            Filter
          </legend>
        ...
        <div className="grid grid-cols-[50px_auto_auto_65px_auto] justify-end items-center gap-3 p-3 lg:p-5 text-emberSoft">
          <span>Sort By:</span>
          <select
            value={sortBy}
          ...
          <div className="w-[2px] h-full bg-ember" />
