  ...
  return (
    ...
    >
      <div className="min-w-0">
        <p className={status === "done" ? "text-bark line-through" : "text-ink"}>
          {todo?.text || "No text"}
        </p>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          >
            {statusOptions.find((option) => option.value === status)?.label || "To do"}
          </span>
          <span
          ...
          {(todo?.tags || []).map((tag) => (
            <span key={`${todo?._id}-${tag}`} className="rounded-full bg-emberSoft px-2.5 py-1 text-xs text-ink">
              #{tag}
            </span>
          ))}
        </div>
      ...
      <div className="flex items-center gap-2 sm:justify-end">
        ...
        <select
          id={`status-${todo?._id}`}
          value={status}
          onChange={(event) => onStatusChange(todo, event.target.value as TodoStatus)}
