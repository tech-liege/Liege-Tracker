...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    setStatus({
      title: "Authentication",
      detail: authMode === "login" ? "Sign in to continue" : "Create an account",
      meta: "Guest",
    });
  ...
  return (
    <section className="ui-enter rounded-3xl border border-border bg-white/90 p-6 shadow-soft backdrop-blur sm:p-8">
      ...
      <form onSubmit={handleAuthSubmit} className="mt-6 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
        <input
          type="email"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 px-4">
          <div className="w-full max-w-md rounded-3xl border border-border bg-white p-6 shadow-soft">
            ...
            <p className="mt-2 text-sm text-bark">
              We found {pendingGuestSync?.guestTodos.length || 0} local guest task
              {(pendingGuestSync?.guestTodos.length || 0) === 1 ? "" : "s"}. Do you want to sync
